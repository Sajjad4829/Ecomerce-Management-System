...
export default function WarrantyPolicyCenter() {
  const { warrantyPolicies } = useAfterSales();

  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-serif text-neutral-900">Warranty Policies</h1>
          <p className="text-sm text-neutral-500 mt-1">Configure warranty durations and coverage</p>
        </div>
          ...
          <tbody className="divide-y divide-neutral-200">
            {warrantyPolicies.map(wp => (
              <tr key={wp.id} className="hover:bg-neutral-50">
                <td className="px-6 py-4 font-medium text-neutral-900">{wp.name}</td>
                <td className="px-6 py-4 text-neutral-600">{wp.category}</td>
