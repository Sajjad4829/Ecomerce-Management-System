    ...
    <div className="flex flex-col min-h-[calc(100vh-theme(spacing.20))] pb-24">
      <div className="sticky top-0 z-20 bg-background pt-4 pb-4 border-b border-black/5 flex flex-col md:flex-row md:items-center justify-between gap-4 shrink-0">
        <div className="flex items-center gap-4">
          <Link to="/admin/customers/groups" className="p-2 bg-surface border border-black/10 rounded-lg text-text-muted hover:text-black hover:border-black/20 transition-all shadow-sm">
            <FiArrowLeft size={18} />
          </Link>
          <div>
        ...
        <div className="flex items-center gap-3">
          <Link to="/admin/customers/groups" className="px-4 py-2 text-text-secondary hover:text-black text-sm font-medium transition-colors">
            Cancel
          </Link>
          <button 
            onClick={handleSave}
      ...
      <div className="max-w-4xl mx-auto w-full mt-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-surface rounded-xl border border-black/5 shadow-sm p-8">
            <h2 className="text-lg font-serif font-bold text-text-primary mb-6">Group Information</h2>
            
            <div className="space-y-6">
              <div>
                <label className="block text-xs font-mono font-bold text-text-muted uppercase mb-2">Group Name *</label>
                <input 
                  type="text" 
                  value={formData.name}
                  onChange={(e) => {
                  ...
                  className={`w-full px-4 py-2.5 bg-background border ${errors.name ? 'border-red-500' : 'border-transparent'} rounded-lg focus:outline-none focus:ring-2 focus:ring-black/20 text-sm font-medium text-text-primary`}
                />
          ...
          <div className="bg-surface rounded-xl border border-black/5 shadow-sm p-8">
            <h2 className="text-lg font-serif font-bold text-text-primary mb-6">Commerce & Pricing</h2>
            ...
            <div>
              <label className="block text-xs font-mono font-bold text-text-muted uppercase mb-2">Assigned Pricing Rule</label>
              <select 
        ...
        <div className="space-y-6">
          <div className="bg-surface rounded-xl border border-black/5 shadow-sm p-6">
            <h3 className="text-sm font-bold text-text-primary mb-4">Status</h3>
            <select 
