      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium text-text-primary">Groups</span>
          </div>
          <h1 className="text-3xl font-serif font-bold text-text-primary mt-2">Customer Groups</h1>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-background border-b border-black/5">
                ...
                <th className="p-4 text-[10px] font-mono font-bold text-text-muted uppercase tracking-wider">Customers</th>
                <th className="p-4 text-[10px] font-mono font-bold text-text-muted uppercase tracking-wider">Pricing / Discount</th>
                ...
                <tr key={group.id} className="hover:bg-background transition-colors">
                  ...
                  <td className="p-4">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-medium text-text-primary">{group.count}</span>
                    </div>
                  </td>
                  <td className="p-4">
                    <div className="flex items-center gap-2">
                      <FiTag className="text-text-muted" size={14} />
                      <span className="text-sm text-text-secondary">{group.discount}</span>
                    </div>
                  </td>
                  <td className="p-4">
                    <span className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider border ${
                      group.status === 'Active' ? 'bg-success-soft text-green-800 border-green-200' : 'bg-gray-100 text-gray-800 border-border'
                    }`}>
                      {group.status}
                    </span>
                  </td>
                  <td className="p-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <Link to={`/admin/customers/groups/${group.id}`} className="p-2 text-text-muted hover:text-black hover:bg-gray-100 rounded-lg transition-colors">
                        <FiEdit size={16} />
                      </Link>
