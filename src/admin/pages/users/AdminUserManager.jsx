  ...
  return (
    <div className="space-y-8 pb-12">
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
        <div>
          <h1 className="text-3xl font-serif font-bold text-text-primary mb-2">Workspace Users</h1>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-background border-b border-black/5">
                <th className="p-4 text-[10px] font-mono font-bold text-text-muted uppercase tracking-wider">User</th>
                ...
                <th className="p-4 text-[10px] font-mono font-bold text-text-muted uppercase tracking-wider">Last Login</th>
                ...
                <tr key={user.id} className="hover:bg-background transition-colors">
                  ...
                  <td className="p-4">
                    <div className="flex items-center gap-2 text-sm text-text-secondary font-medium">
                      <FiShield className="text-text-muted" /> {user.role}
                    </div>
                  </td>
                  <td className="p-4">
                    <span className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider border ${
                      user.status === 'Active' ? 'bg-success-soft text-green-800 border-green-200' : 'bg-gray-100 text-gray-800 border-border'
                    }`}>
                      {user.status}
                    </span>
                  </td>
                  ...
                  <td className="p-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <PermissionGate permission="users.manage" fallback={<span className="text-xs text-text-muted">View Only</span>}>
                        <Link to={`/admin/users/${user.id}/edit`} className="p-2 text-text-muted hover:text-black hover:bg-gray-100 rounded-lg transition-colors">
                          <FiEdit2 size={16} />
                        </Link>
