  ...
  return (
    <div className="max-w-7xl mx-auto">
      <div className="flex items-center gap-4 mb-8">
        <button onClick={() => navigate('/admin/settings/roles')} className="text-text-muted hover:text-black">
          <FiArrowLeft className="w-5 h-5" />
        </button>
        <div>
          <div className="flex items-center gap-3">
            <h1 className="text-2xl font-serif font-bold text-text-primary">{role.name}</h1>
            {role.isSystem && (
          ...
          <p className="text-text-muted text-sm mt-1">{role.description}</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-surface p-6 rounded-xl border border-black/5 shadow-sm">
          <div className="flex items-center gap-4 mb-2">
            ...
            <h3 className="font-medium text-text-muted">Staff Assigned</h3>
          </div>
        ...
        <div className="bg-surface p-6 rounded-xl border border-black/5 shadow-sm">
          <div className="flex items-center gap-4 mb-2">
            ...
            <h3 className="font-medium text-text-muted">Permissions</h3>
          </div>
          <p className="text-3xl font-bold">{role.permissionCount}</p>
        </div>
        <div className="bg-surface p-6 rounded-xl border border-black/5 shadow-sm flex flex-col justify-center">
           <p className="text-sm text-text-muted mb-1">Role Status</p>
           <div className="flex items-center gap-2">
             <span className={`w-2.5 h-2.5 rounded-full ${role.status === 'Active' ? 'bg-success-soft0' : 'bg-gray-400'}`}></span>
      ...
      <div className="bg-surface rounded-xl border border-black/5 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-black/5 flex justify-between items-center bg-background/50">
          <div>
            <h3 className="font-medium text-lg">Permission Matrix</h3>
            <p className="text-sm text-text-muted mt-1">Configure access levels across modules and resources</p>
          </div>
          <button 
            onClick={() => navigate(`/admin/settings/roles/${roleId}/permissions`)}
            className="px-4 py-2 border border-black/10 rounded-lg bg-surface hover:bg-background"
          >
            Edit Matrix
          </button>
        </div>
