  ...
  if (!shipment) {
    return (
      <div className="p-8 text-center">
        ...
        <button onClick={() => navigate('/admin/shipments')} className="text-blue-600 hover:underline mt-2">Return to shipments</button>
      </div>
    );
  }
    ...
    <div className="space-y-6 max-w-6xl mx-auto pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <Link to="/admin/shipments" className="p-2 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">Order: <Link to={`/admin/orders/${shipment.orderId}`} className="text-blue-600 hover:underline">{shipment.orderId}</Link></p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
            <div className="flex justify-between items-start mb-6">
              <div>
                <h2 className="text-lg font-bold text-gray-900 flex items-center gap-2"><FiTruck /> Carrier Information</h2>
                <p className="text-sm text-gray-500 mt-1">Managed by {shipment.carrier}</p>
              </div>
              <div className="text-right">
                <p className="text-sm text-gray-500 uppercase tracking-wider font-semibold">Tracking Number</p>
                <p className="text-lg font-mono font-bold text-gray-900 mt-1">{shipment.trackingNumber}</p>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4 border-t border-gray-100 pt-4">
              <div>
                <p className="text-xs text-gray-500 uppercase">Shipping Method</p>
                <p className="text-sm font-medium text-gray-900">{shipment.shippingMethod}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500 uppercase">Created On</p>
                <p className="text-sm font-medium text-gray-900">{new Date(shipment.createdAt).toLocaleString()}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
            <div className="flex justify-between items-center mb-6">
               <h2 className="text-lg font-bold text-gray-900 flex items-center gap-2"><FiCalendar /> Delivery Scheduling</h2>
               <button onClick={() => setIsSchedulerOpen(true)} className="text-sm text-blue-600 hover:underline">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div className="bg-gray-50 rounded-lg p-4 border border-gray-100">
                <p className="text-xs text-gray-500 uppercase font-semibold mb-2">Estimated Delivery</p>
                <p className="text-lg font-medium text-gray-900">{new Date(shipment.estimatedDelivery).toLocaleDateString()}</p>
                <p className="text-sm text-gray-500 mt-1">End of day</p>
              </div>
              
              <div className={`rounded-lg p-4 border ${shipment.scheduledDelivery ? 'bg-blue-50 border-blue-100' : 'bg-gray-50 border-gray-100'}`}>
                ...
                {shipment.scheduledDelivery ? (
                  ...
                  <p className="text-sm text-gray-500 italic mt-2">No specific slot scheduled.</p>
                )}
              </div>
            </div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-200">
              <h2 className="text-lg font-bold text-gray-900 flex items-center gap-2"><FiBox /> Packages ({shipment.packages.length})</h2>
            </div>
            <div className="divide-y divide-gray-100">
              {shipment.packages.map(pkg => (
                <div key={pkg.id} className="p-6">
                  <div className="flex justify-between items-start mb-4">
                    <h3 className="font-medium text-gray-900">Package {pkg.number}</h3>
                    <div className="flex gap-2">
                      {pkg.fragile && <span className="px-2 py-0.5 bg-red-100 text-red-800 text-xs rounded-full font-medium">Fragile</span>}
                      {pkg.oversized && <span className="px-2 py-0.5 bg-purple-100 text-purple-800 text-xs rounded-full font-medium">Oversized</span>}
                    </div>
                  </div>
                  <p className="text-sm text-gray-600 mb-4">{pkg.contents}</p>
                  <div className="flex flex-wrap gap-x-8 gap-y-4">
                    <div>
                      <p className="text-xs text-gray-500 uppercase">Weight</p>
                      <p className="text-sm font-medium text-gray-900">{pkg.weight} lbs</p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-500 uppercase">Dimensions (L x W x H)</p>
                      <p className="text-sm font-medium text-gray-900">{pkg.length}" x {pkg.width}" x {pkg.height}"</p>
                    </div>
                  </div>
                </div>
              ))}
        ...
        <div className="space-y-6">
          {/* Destination */}
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
            <h2 className="text-lg font-bold text-gray-900 mb-4 flex items-center gap-2"><FiMapPin /> Destination</h2>
            <div className="space-y-1">
              <p className="font-medium text-gray-900">{shipment.destination.recipient}</p>
              ...
              <p className="text-sm text-gray-500 mt-3 pt-3 border-t border-gray-100">Phone: <span className="text-gray-900">{shipment.destination.phone}</span></p>
            </div>
          </div>
