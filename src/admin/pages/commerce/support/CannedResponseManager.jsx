    ...
    <div className="space-y-6 max-w-6xl mx-auto pb-12">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Manage quick replies and message templates for agents.</p>
        </div>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {cannedResponses.map(resp => (
              <tr key={resp.id} className="hover:bg-gray-50">
                <td className="px-6 py-4 whitespace-nowrap font-medium text-gray-900">{resp.title}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{resp.category}</td>
                <td className="px-6 py-4">
                  <p className="text-sm text-gray-600 line-clamp-1">{resp.message}</p>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                    resp.status === 'Active' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'
                  }`}>
                    {resp.status}
                  </span>
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-right text-sm">
                   <button className="text-gray-400 hover:text-blue-600 mr-3"><FiEdit2 size={16} /></button>
