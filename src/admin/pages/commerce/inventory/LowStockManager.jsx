      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
        <div>
          <div className="flex items-center gap-2">
            <span className="px-2 py-0.5 rounded bg-danger-soft text-red-900 font-mono text-[10px] uppercase font-bold flex items-center gap-1">
              <FiAlertTriangle size={10} /> Action Required
            </span>
          </div>
          <h1 className="text-3xl font-serif font-bold text-text-primary mt-2">Low Stock Alerts</h1>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-background border-b border-black/5">
                <th className="p-4 text-[10px] font-mono font-bold text-text-muted uppercase tracking-wider">Severity</th>
                ...
                <th className="p-4 text-[10px] font-mono font-bold text-text-muted uppercase tracking-wider text-right">Current Stock</th>
                <th className="p-4 text-[10px] font-mono font-bold text-text-muted uppercase tracking-wider text-right">Incoming</th>
                ...
                <tr key={item.id} className="hover:bg-background transition-colors group">
                  <td className="p-4">
                    {item.status === 'out-of-stock' ? (
                      <span className="inline-flex items-center px-2 py-1 bg-danger-soft text-red-700 text-xs font-bold rounded">
                        Critical
                      </span>
                    ) : (
                      <span className="inline-flex items-center px-2 py-1 bg-warning-soft text-warning text-xs font-bold rounded">
                        Warning
                      </span>
                    )}
                  </td>
                  ...
                  <td className="p-4 text-right">
                    <div className="flex flex-col items-end">
                      <span className={`text-xl font-bold ${item.onHand === 0 ? 'text-danger' : 'text-text-primary'}`}>
                        {item.onHand}
                      </span>
                  ...
                  <td className="p-4 text-right">
                    <span className="text-sm font-bold text-primary">{item.incoming > 0 ? `+${item.incoming}` : '-'}</span>
                  </td>
                  <td className="p-4 text-right">
                    <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button className="text-xs font-medium text-text-secondary hover:text-text-primary bg-gray-100 px-3 py-1.5 rounded-md flex items-center gap-1">
