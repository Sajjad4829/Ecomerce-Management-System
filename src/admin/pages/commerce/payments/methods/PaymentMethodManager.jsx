    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-text-muted mt-1">Manage methods available to customers at checkout.</p>
        </div>
      </div>
      ...
      <div className="bg-surface rounded-lg shadow-sm border border-border overflow-hidden">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-background">
            <tr>
              <th className="px-6 py-3 text-left text-xs font-medium text-text-muted uppercase tracking-wider">Method Name</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-text-muted uppercase tracking-wider">Provider / Type</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-text-muted uppercase tracking-wider">Sort Order</th>
          ...
          <tbody className="bg-surface divide-y divide-gray-200">
            {MOCK_METHODS.map((method) => (
              <tr key={method.id} className="hover:bg-background">
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-text-primary">{method.name}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-text-muted">
                  <div className="font-medium text-text-primary">{method.provider}</div>
                  <div className="text-xs">{method.type}</div>
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-text-muted">{method.sortOrder}</td>
                <td className="px-6 py-4 whitespace-nowrap">
                  {method.status === 'Active' ? (
                    <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium bg-success-soft text-green-800">
