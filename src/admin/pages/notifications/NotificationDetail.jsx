...
export default function NotificationDetail() {
  ...
  if (!notification) {
    return <div className="p-12 text-center text-text-muted">Notification not found.</div>;
  }
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 pb-24">
      <div className="flex items-center justify-between">
        <Link to="/admin/notifications" className="flex items-center gap-2 text-text-muted hover:text-text-primary transition-colors">
          <FiArrowLeft /> Back to Notifications
        </Link>
        <div className="flex gap-2">
          {notification.status === 'Unread' && (
            <button onClick={() => markAsRead(notification.id)} className="flex items-center gap-2 px-3 py-1.5 border border-border text-text-secondary bg-surface hover:bg-background rounded-lg text-sm font-medium">
              <FiCheckCircle size={16} /> Mark as Read
            </button>
          )}
          <button onClick={handleArchive} className="flex items-center gap-2 px-3 py-1.5 border border-border text-text-secondary bg-surface hover:bg-background rounded-lg text-sm font-medium">
      ...
      <div className="bg-surface rounded-xl border border-border shadow-sm overflow-hidden">
        <div className="p-6 border-b border-border">
          <div className="flex items-center gap-3 mb-4">
            <span className="px-2.5 py-1 bg-stone-100 text-text-secondary rounded-full text-xs font-medium border border-border">
              {notification.type}
            </span>
            <span className={`px-2.5 py-1 rounded-full text-xs font-bold uppercase tracking-wider border ${
              notification.priority === 'Critical' ? 'bg-danger-soft text-red-700 border-red-200' :
              ...
              'bg-stone-100 text-text-secondary border-border'
            }`}>
