    ...
    <div className="space-y-6 max-w-5xl mx-auto p-6">
      ...
      <div className="flex items-center justify-between pb-4 border-b border-stone-200">
        <div className="flex items-center gap-3">
          <button
            onClick={onBack}
            className="p-2 text-stone-500 hover:text-stone-900 bg-stone-100 rounded-lg transition-colors"
          >
            <FiArrowLeft size={18} />
          </button>
          <div>
            <h1 className="font-serif font-bold text-2xl text-stone-900">Template Assignment Engine</h1>
      ...
      <div className="bg-white rounded-xl border border-stone-200 p-6 shadow-sm space-y-5">
        <div>
          <h2 className="font-serif font-bold text-base text-stone-900">Global Default Templates</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="p-4 bg-stone-50 border border-stone-200 rounded-xl space-y-2">
            ...
            <select
              ...
              className="w-full px-3 py-2 bg-white border border-stone-300 rounded-lg text-xs font-semibold text-stone-900 focus:ring-2 focus:ring-stone-900"
            >
      ...
      <div className="bg-white rounded-xl border border-stone-200 p-6 shadow-sm space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="font-serif font-bold text-base text-stone-900">Priority Override Rules</h2>
            <p className="text-xs text-stone-500">
        ...
        <div className="space-y-3">
          {overrides.map((ov) => (
            <div key={ov.id} className="p-4 bg-stone-50 rounded-xl border border-stone-200 flex flex-wrap items-center justify-between gap-4 text-xs">
              <div className="flex items-center gap-3">
                <span className="px-2 py-1 bg-amber-200 text-amber-900 font-mono font-bold rounded text-[10px]">
                  Priority {ov.priority}
                </span>
                <div>
                  <div className="font-semibold text-stone-900">{ov.pageType}</div>
                  <div className="text-stone-500 font-mono text-[11px]">When {ov.condition}</div>
                </div>
              </div>

              <div className="flex items-center gap-3">
                <span className="text-stone-400 font-mono">Use Template:</span>
                <select
                  value={ov.templateId}
                  onChange={(e) => {
                  ...
                  className="px-3 py-1.5 bg-white border border-stone-300 rounded-lg text-xs font-semibold text-stone-900"
                >
                  {templates.map((t) => (
                    <option key={t.id} value={t.id}>
                      {t.name}
