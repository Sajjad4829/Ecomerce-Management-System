...
export const RefundCenter = () => {
  const { refunds } = useFinance();
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-serif text-neutral-900">Refunds</h1>
          <p className="text-sm text-neutral-500 mt-1">Manage requested and completed refunds</p>
        </div>
      </div>
            ...
            <tbody className="divide-y divide-neutral-200">
              {filteredRefunds.map((ref) => (
                <tr key={ref.id} className="hover:bg-neutral-50 transition-colors">
                  <td className="px-6 py-4 font-medium text-neutral-900">{ref.id}</td>
                  <td className="px-6 py-4 text-indigo-600 hover:underline cursor-pointer">{ref.orderId}</td>
                  <td className="px-6 py-4 text-neutral-600">{ref.customer}</td>
                  <td className="px-6 py-4 text-neutral-600">{ref.reason}</td>
                  ...
                  <td className="px-6 py-4 text-right">
                    <button 
                      onClick={() => navigate(`/admin/finance/refunds/${ref.id}`)}
