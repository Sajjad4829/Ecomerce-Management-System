    ...
    <div className="flex flex-col min-h-[calc(100vh-theme(spacing.20))] pb-24">
      <div className="sticky top-0 z-20 bg-[#F7F5F2] pt-4 pb-4 border-b border-black/5 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-4">
          <Link to="/admin/seo/redirects" className="p-2 bg-white border border-black/10 rounded-lg text-gray-500 hover:text-black hover:border-black/20 transition-all shadow-sm">
            <FiArrowLeft size={18} />
          </Link>
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">Create a new URL forwarding rule</p>
          </div>
        </div>
        
        <div className="flex items-center gap-3">
          <Link to="/admin/seo/redirects" className="px-4 py-2 text-gray-600 hover:text-black text-sm font-medium transition-colors">
            Cancel
          </Link>
          <button 
            onClick={handleSave}
      ...
      <div className="max-w-3xl mx-auto w-full mt-8">
        <div className="bg-white rounded-xl border border-black/5 shadow-sm p-8 space-y-6">
          
          <div>
            <label className="block text-xs font-mono font-bold text-gray-500 uppercase mb-2">Source URL (Old Path)</label>
            <div className="flex">
              ...
              <input 
                ...
                className={`w-full px-4 py-2.5 bg-[#F7F5F2] border ${errors.source ? 'border-red-500' : 'border-transparent'} rounded-r-lg focus:outline-none focus:ring-2 focus:ring-black/20 text-sm font-medium text-[#1A1A1A]`}
              />
            </div>
          ...
          <div>
            <label className="block text-xs font-mono font-bold text-gray-500 uppercase mb-2">Destination URL (New Path)</label>
            <input 
              type="text" 
              ...
              className={`w-full px-4 py-2.5 bg-[#F7F5F2] border ${errors.destination ? 'border-red-500' : 'border-transparent'} rounded-lg focus:outline-none focus:ring-2 focus:ring-black/20 text-sm font-medium text-[#1A1A1A]`}
            />
          ...
          <div className="grid grid-cols-2 gap-6 pt-4 border-t border-black/5">
            <div>
              <label className="block text-xs font-mono font-bold text-gray-500 uppercase mb-2">Redirect Type</label>
              <select 
                value={formData.type}
