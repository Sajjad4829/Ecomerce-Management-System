...
export default function DiscountEditor({ formData, setFormData }) {
  return (
    <div className="space-y-8">
      <div>
        <h2 className="text-lg font-serif font-bold text-[#1A1A1A] mb-1">Discount Value</h2>
        <p className="text-sm text-gray-500">How much will be deducted from the price.</p>
      </div>

      <div className="space-y-6">
        <div>
          <label className="block text-xs font-mono font-bold text-gray-500 uppercase mb-3">Discount Type</label>
            ...
            <button
              ...
              className={`flex-1 py-3 px-4 rounded-xl border flex items-center justify-center gap-2 text-sm font-bold transition-all ${
                formData.discountType === 'percentage'
                  ? 'border-[#1A1A1A] bg-gray-50 shadow-sm text-[#1A1A1A]'
        ...
        <div>
          ...
          <div className="relative w-1/2">
            ...
            <input 
              ...
              className={`w-full pr-4 py-2.5 bg-[#F7F5F2] border-transparent rounded-lg focus:outline-none focus:ring-2 focus:ring-black/20 focus:bg-white transition-all text-sm font-medium text-[#1A1A1A] ${
