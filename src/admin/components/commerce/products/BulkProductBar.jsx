    ...
    <AnimatePresence>
      {selectedCount > 0 && (
        <motion.div 
          ...
          className="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 bg-primary text-white px-4 py-3 rounded-xl shadow-2xl flex items-center gap-4 border border-stone-700"
        >
          <div className="flex items-center gap-3 pr-4 border-r border-stone-700">
            <div className="w-6 h-6 rounded-full bg-surface text-text-primary flex items-center justify-center text-xs font-bold">
              {selectedCount}
            </div>
            <span className="text-sm font-medium">Selected</span>
            <button onClick={onClear} className="text-text-muted hover:text-white transition-colors p-1">
              <FiX size={16} />
            </button>
          </div>
