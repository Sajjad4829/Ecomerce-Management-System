          ...
          >
            <div className="flex items-center justify-between p-4 border-b border-stone-100 bg-stone-50/50">
              <div className="flex items-center gap-2">
                <h2 className="font-serif font-bold text-stone-900 text-lg">Category Preview</h2>
            ...
            <div className="flex-1 overflow-y-auto">
              <div className="aspect-[21/9] bg-stone-100 relative border-b border-stone-200 flex items-center justify-center">
                {category.image ? (
                  <img src={category.image} alt={category.name} className="w-full h-full object-cover" />
                ) : (
                  <span className="text-stone-400 font-serif">No Banner Image</span>
                )}
              </div>

              <div className="p-6 space-y-6">
                <div>
                  <h1 className="font-serif text-2xl font-bold text-stone-900">{category.name}</h1>
                  <p className="font-mono text-xs text-stone-500 mt-1">/{category.slug}</p>
                </div>

                <div className="grid grid-cols-2 gap-4 pt-4 border-t border-stone-100">
                  <div>
                    <p className="text-[10px] font-mono font-bold text-stone-500 uppercase">Products</p>
                    <p className="text-sm text-stone-900 mt-0.5">{category.productCount} Assigned</p>
                  </div>
                  <div>
                    <p className="text-[10px] font-mono font-bold text-stone-500 uppercase">Subcategories</p>
                    <p className="text-sm text-stone-900 mt-0.5">{category.children ? category.children.length : 0}</p>
                  </div>
                  <div>
                    <p className="text-[10px] font-mono font-bold text-stone-500 uppercase">Featured</p>
                    <p className="text-sm text-stone-900 mt-0.5">{category.featured ? 'Yes' : 'No'}</p>
                  </div>
                  <div>
                    <p className="text-[10px] font-mono font-bold text-stone-500 uppercase">Last Updated</p>
                    <p className="text-sm text-stone-900 mt-0.5">{category.updatedAt}</p>
                  </div>
                </div>
              </div>
            </div>
