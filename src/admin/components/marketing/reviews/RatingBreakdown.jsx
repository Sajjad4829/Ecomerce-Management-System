...
export default function RatingBreakdown({ distribution, totalReviews, averageRating }) {
  ...
  return (
    <div className="flex flex-col md:flex-row gap-8 items-center bg-white p-6 border border-black/5 rounded-xl shadow-sm">
      <div className="text-center md:text-left flex flex-col items-center md:items-start shrink-0">
        <h3 className="text-sm font-bold text-[#1A1A1A] mb-2">Customer Reviews</h3>
        ...
        {[5, 4, 3, 2, 1].map((stars) => {
          ...
          return (
            <div key={stars} className="flex items-center gap-3 text-sm">
              ...
              <span className="w-12 text-right text-xs text-gray-500 shrink-0">({count})</span>
            </div>
          );
        })}
      </div>
