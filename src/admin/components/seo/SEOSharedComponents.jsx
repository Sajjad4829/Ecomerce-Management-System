      ...
      <div className="flex gap-2">
        <div className="relative flex-1">
          ...
          <input
            ...
            className={`w-full pl-10 pr-4 py-2 bg-background border ${error ? 'border-red-300' : 'border-border'} rounded-lg text-sm focus:outline-none focus:border-primary`}
          />
        </div>
        <button
          type="button"
          onClick={handleGenerate}
          className="px-4 py-2 bg-stone-100 text-text-secondary text-sm font-medium rounded-lg hover:bg-stone-200 transition-colors whitespace-nowrap"
        >
          Auto-generate
        </button>
      </div>
...
export const CanonicalUrlField = ({ value, onChange, defaultUrl }) => (
  <div className="space-y-3">
    <label className="flex items-center gap-2 cursor-pointer">
      ...
      />
      <span className="text-sm text-text-primary font-medium">Automatic ({defaultUrl || 'default path'})</span>
    </label>
    <label className="flex items-center gap-2 cursor-pointer">
      <input 
        type="radio" 
        ...
        className="text-text-primary focus:ring-stone-900"
      />
      <span className="text-sm text-text-primary font-medium">Custom URL</span>
    </label>
    {!!value && value !== defaultUrl && (
      <input
        ...
        className="w-full mt-2 p-2.5 bg-background border border-border rounded-lg text-sm focus:outline-none focus:border-primary"
      />
    )}
  </div>
);
...
export const SocialPreview = ({ title, description, image, platform = 'og' }) => (
  <div className="bg-surface border border-border rounded-lg overflow-hidden max-w-[500px] shadow-sm">
    <div className="aspect-[1.91/1] bg-stone-100 flex items-center justify-center relative border-b border-border">
      {image ? (
        <img src={image} alt="Social Preview" className="w-full h-full object-cover" />
      ) : (
        <span className="text-text-muted text-sm font-medium">No Image Selected</span>
      )}
    </div>
    <div className="p-4 bg-[#f2f3f5]">
      <div className="text-xs text-text-muted uppercase tracking-wide mb-1">aurorafurniture.com</div>
...
export const SEOContentOptimizer = ({ seoData, content }) => {
  ...
  return (
    <div className="bg-background rounded-lg p-5 border border-border">
      <h4 className="font-bold text-text-primary mb-4 text-sm">Content Optimization Guide</h4>
      
      <div className="space-y-4">
        <div>
          <div className="flex justify-between text-xs mb-1">
            <span className="text-text-secondary font-medium">Title Length</span>
            <span className={analysis.titleScore > 80 ? 'text-success' : 'text-amber-500'}>{seoData?.title?.length || 0}/60</span>
          </div>
          <div className="h-1.5 w-full bg-stone-200 rounded-full overflow-hidden">
             <div className={`h-full ${analysis.titleScore > 80 ? 'bg-success-soft0' : 'bg-amber-400'}`} style={{ width: `${Math.min(100, ((seoData?.title?.length || 0)/60)*100)}%` }}></div>
          </div>
        </div>

        <div>
          <div className="flex justify-between text-xs mb-1">
            <span className="text-text-secondary font-medium">Description Length</span>
            <span className={analysis.descriptionScore > 80 ? 'text-success' : 'text-amber-500'}>{seoData?.description?.length || 0}/160</span>
          </div>
          <div className="h-1.5 w-full bg-stone-200 rounded-full overflow-hidden">
             <div className={`h-full ${analysis.descriptionScore > 80 ? 'bg-success-soft0' : 'bg-amber-400'}`} style={{ width: `${Math.min(100, ((seoData?.description?.length || 0)/160)*100)}%` }}></div>
          </div>
        </div>

        <div className="pt-4 border-t border-border">
          <h5 className="text-xs font-bold text-text-primary mb-2">Recommendations</h5>
