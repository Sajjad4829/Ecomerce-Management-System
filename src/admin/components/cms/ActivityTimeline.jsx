...
export default function ActivityTimeline() {
  return (
    <div className="bg-surface border border-black/5 rounded-xl p-6 shadow-sm">
      <h3 className="text-[10px] uppercase tracking-widest text-text-muted mb-6">Recent Activity</h3>
        ...
        {MOCK_ACTIVITIES.map((activity, index) => {
          ...
          return (
            ...
            >
              ...
              {/* Card */}
              <div className="w-[calc(100%-2.5rem)] md:w-[calc(50%-1.5rem)] bg-background p-4 rounded-lg border border-black/5 hover:border-black/10 transition-colors">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-xs font-bold text-text-primary">{activity.action}</span>
                  <span className="text-[10px] text-text-muted font-medium">{activity.time}</span>
                </div>
                <div className="text-sm text-text-secondary font-serif italic">{activity.target}</div>
              </div>
            </motion.div>
          );
        })}
      ...
      <button className="w-full mt-6 py-3 border border-black/10 rounded-lg text-xs font-semibold uppercase tracking-widest text-text-secondary hover:bg-background transition-colors">
        View All Activity
      </button>
    </div>
  );
