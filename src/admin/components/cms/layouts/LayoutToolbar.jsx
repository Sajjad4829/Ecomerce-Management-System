  ...
  return (
    <div className="bg-surface p-4 rounded-xl border border-black/5 flex flex-col md:flex-row gap-4 justify-between items-center shadow-sm sticky top-0 z-20 mb-6">
      <div className="flex items-center gap-4 w-full md:w-auto">
        <h2 className="text-lg font-bold font-serif text-text-primary capitalize">{activeSection.replace('-', ' ')}</h2>
        <div className="h-6 w-px bg-black/10"></div>
        <div className="text-xs font-semibold text-text-muted bg-gray-100 px-2 py-1 rounded">
          Active Layout: <span className="text-text-primary">Luxury Commerce</span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 w-full md:w-auto justify-end">
        ...
        <button className="p-2 border border-black/10 rounded-lg text-text-secondary hover:bg-background transition-colors" title="Export Layout">
          <FiDownload size={16} />
        </button>
        <button className="p-2 border border-black/10 rounded-lg text-text-secondary hover:bg-background transition-colors" title="Import Layout">
          <FiUpload size={16} />
        </button>
