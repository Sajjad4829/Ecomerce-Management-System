...
const TreeItem = ({ item, depth = 0, isSelected, onSelect }) => {
  ...
  const getIcon = () => {
    ...
    return <FiLink size={14} className="text-gray-400" />;
  };
    ...
    <div className="select-none">
      <div 
        className={cn(
          ...
          isSelected ? "bg-blue-50 border-blue-200" : "bg-white border-black/5 hover:border-black/10 hover:bg-gray-50"
        )}
      ...
      >
        ...
        <div 
          className="w-5 h-5 flex items-center justify-center cursor-pointer text-gray-400 hover:text-[#1A1A1A] rounded hover:bg-black/5"
        ...
        <div className="text-sm font-semibold text-[#1A1A1A] flex-1 truncate">
          {item.label}
        </div>
        ...
        <button className="p-1.5 text-gray-400 hover:text-[#1A1A1A] rounded hover:bg-black/5 opacity-0 group-hover:opacity-100 transition-opacity">
  ...
  return (
    <div className="bg-white border border-black/5 rounded-xl flex flex-col h-full overflow-hidden shadow-sm">
      <div className="p-4 border-b border-black/5 shrink-0 bg-gray-50/50 flex justify-between items-center">
        <h3 className="text-sm font-bold text-[#1A1A1A] uppercase tracking-widest">Navigation Structure</h3>
        <span className="text-xs text-gray-500 font-medium">Drag to reorder</span>
      </div>

      <div className="flex-1 overflow-y-auto custom-scrollbar p-6 bg-[#F7F5F2]">
        <div className="max-w-2xl mx-auto space-y-1">
          ...
          <div className="mt-6 border-2 border-dashed border-black/10 rounded-xl p-8 flex flex-col items-center justify-center text-center bg-gray-50/50">
             <div className="text-sm font-semibold text-gray-400 mb-1">Drag items here</div>
             <p className="text-xs text-gray-400">Add new navigation links from the library</p>
          </div>
        </div>
      </div>
    </div>
