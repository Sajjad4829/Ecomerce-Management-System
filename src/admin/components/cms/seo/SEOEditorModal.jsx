...
export default function SEOEditorModal({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-xs flex justify-end animate-in fade-in duration-200">
      ...
      {/* Main Drawer Panel */}
      <div className="w-full max-w-3xl bg-surface h-full shadow-2xl flex flex-col border-l border-black/10 animate-in slide-in-from-right duration-300">
        ...
        <div className="p-4 border-b border-black/5 flex items-center justify-between bg-background/50 shrink-0">
          <div>
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-serif font-bold text-base text-text-primary truncate max-w-sm">
        ...
        {/* Navigation Tabs */}
        <div className="flex items-center border-b border-black/5 px-6 bg-surface shrink-0 text-xs font-bold text-text-muted overflow-x-auto">
          {[
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  activeTab === tab.id
                    ? "border-[#1A1A1A] text-text-primary font-bold"
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-6 bg-background/30">
          ...
          {activeTab === 'basic' && (
            <div className="space-y-6">
              ...
              {/* Form Input Fields */}
              <div className="bg-surface p-5 border border-black/10 rounded-xl shadow-2xs space-y-4">
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-text-secondary uppercase tracking-wider flex items-center justify-between">
                    ...
                    <span className="text-[10px] text-text-muted font-mono">{formData.title.length} / 60 chars</span>
                  </label>
                  <input
                    type="text"
                    value={formData.title}
                    onChange={(e) => setFormData({ ...formData, title: e.target.value })}
                    className="w-full px-3 py-2 bg-background border border-black/10 rounded-lg text-xs font-medium focus:bg-surface focus:outline-none focus:border-black/30"
                  />
                </div>
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-text-secondary uppercase tracking-wider flex items-center justify-between">
                    ...
                    <span className="text-[10px] text-text-muted font-mono">{formData.metaDesc.length} / 160 chars</span>
                  </label>
                  <textarea
                    rows={3}
              ...
              <div className="bg-surface p-5 border border-black/10 rounded-xl shadow-2xs space-y-4">
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 bg-background border border-black/10 rounded-lg text-xs font-medium focus:bg-surface focus:outline-none"
                  />
                </div>
        ...
        <div className="p-4 border-t border-black/5 bg-surface flex items-center justify-between shrink-0">
          <span className="text-xs text-text-muted">All changes ready for deployment</span>
