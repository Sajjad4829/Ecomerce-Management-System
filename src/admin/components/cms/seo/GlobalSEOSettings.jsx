...
export default function GlobalSEOSettings({ onSelectMediaClick }) {
  ...
  return (
    <div className="bg-surface border border-black/10 rounded-xl overflow-hidden shadow-2xs">
      
      {/* Header Tabs */}
      <div className="flex items-center border-b border-black/5 px-6 bg-background/60 text-xs font-bold text-text-muted">
        <button
          onClick={() => setActiveTab('defaults')}
          className={cn(
            "py-3.5 border-b-2 font-serif transition-all mr-6 flex items-center gap-2 cursor-pointer",
            activeTab === 'defaults' ? "border-[#1A1A1A] text-text-primary font-bold" : "border-transparent hover:text-black"
          )}
        >
          <FiGlobe size={14} />
        ...
        <button
          ...
          className={cn(
            "py-3.5 border-b-2 font-serif transition-all mr-6 flex items-center gap-2 cursor-pointer",
            activeTab === 'org' ? "border-[#1A1A1A] text-text-primary font-bold" : "border-transparent hover:text-black"
          )}
        >
          <FiCode size={14} />
        ...
        <button
          ...
          className={cn(
            ...
            activeTab === 'sitemap' ? "border-[#1A1A1A] text-text-primary font-bold" : "border-transparent hover:text-black"
          )}
        >
          <FiFileText size={14} />
      ...
      <div className="p-6 space-y-6">
          ...
          <div className="space-y-4 max-w-2xl">
            <div className="space-y-1.5">
              ...
              <input
                ...
                className="w-full px-3 py-2 bg-background border border-black/10 rounded-lg text-xs font-bold"
              />
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                className="w-full px-3 py-2 bg-background border border-black/10 rounded-lg text-xs font-mono"
              />
              <span className="text-[10px] text-text-muted block">Use <code>%s</code> as placeholder for page title.</span>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <textarea
                ...
                className="w-full px-3 py-2 bg-background border border-black/10 rounded-lg text-xs resize-none"
              />
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  className="px-3 py-2 bg-surface border border-black/10 rounded-lg text-xs font-bold flex items-center gap-1.5 hover:bg-background cursor-pointer"
                >
        ...
        {activeTab === 'org' && (
          <div className="space-y-4 max-w-2xl">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-xs font-bold text-text-secondary uppercase tracking-wider block mb-1">Organization Legal Name</label>
                <input
                  type="text"
              ...
              <div>
                <label className="text-xs font-bold text-text-secondary uppercase tracking-wider block mb-1">Customer Support Phone</label>
                <input
                  type="text"
                  value={orgData.phone}
            ...
            <div className="grid grid-cols-3 gap-3">
              <div>
                <label className="text-xs font-bold text-text-secondary uppercase block mb-1">Street Address</label>
                <input
                  type="text"
                  value={orgData.street}
                  onChange={(e) => setOrgData({ ...orgData, street: e.target.value })}
                  className="w-full px-3 py-2 bg-background border border-black/10 rounded-lg text-xs"
                />
              </div>
              <div>
                <label className="text-xs font-bold text-text-secondary uppercase block mb-1">City</label>
                <input
                  type="text"
                  value={orgData.city}
              ...
              <div>
                <label className="text-xs font-bold text-text-secondary uppercase block mb-1">ZIP / Country</label>
                <input
                  type="text"
                  value={`${orgData.zip}, ${orgData.country}`}
                  readOnly
                  className="w-full px-3 py-2 bg-gray-100 border border-black/10 rounded-lg text-xs font-mono text-text-muted"
                />
              </div>
            </div>
          </div>
        ...
        {activeTab === 'sitemap' && (
          <div className="space-y-4 max-w-2xl">
            <div className="flex items-center justify-between p-3 bg-background border border-black/10 rounded-xl">
              <div>
                <div className="text-xs font-bold text-text-primary">XML Sitemap Generation</div>
                <div className="text-[11px] text-text-muted">Expose /sitemap.xml automatically to Google & Bing crawlers</div>
              </div>
              <input
                type="checkbox"
            ...
            <div className="space-y-2 pt-2">
              <label className="text-xs font-bold text-text-secondary uppercase tracking-wider block">Included Collections</label>
              <div className="grid grid-cols-2 gap-3 text-xs">
                <label className="flex items-center gap-2 p-2 bg-background border border-black/5 rounded-lg cursor-pointer">
      ...
      <div className="p-4 border-t border-black/5 bg-background/50 flex items-center justify-between">
        <span className="text-xs text-text-muted">Global site settings will apply across all un-overridden resources</span>
        <button
          onClick={handleSave}
