    ...
    <div className="bg-white border border-black/10 rounded-xl p-5 shadow-2xs space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-black/5 pb-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h4 className="font-serif font-bold text-base text-[#1A1A1A]">Structured Data Schema Builder</h4>
          </div>
        ...
        {/* Type Select */}
        <div className="flex items-center gap-2">
          <label className="text-xs font-bold text-gray-600 uppercase">Schema Type:</label>
          <select
            value={schemaType}
            onChange={(e) => setSchemaType(e.target.value)}
            className="px-3 py-1.5 bg-gray-50 border border-black/10 rounded-lg text-xs font-bold text-[#1A1A1A] focus:bg-white focus:outline-none"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div className="space-y-4">
          ...
          {schemaType === 'Product' && (
            <div className="space-y-3 bg-gray-50/60 p-4 border border-black/5 rounded-xl text-xs space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-2.5 py-1.5 bg-white border border-black/10 rounded-lg text-xs font-medium"
                  />
                </div>
                <div>
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <div>
                  <label className="font-bold text-gray-700 block mb-1">Price ({productFields.currency})</label>
                  <input
                    type="text"
                    value={productFields.price}
                    onChange={(e) => setProductFields({ ...productFields, price: e.target.value })}
                    className="w-full px-2.5 py-1.5 bg-white border border-black/10 rounded-lg text-xs font-mono font-bold"
                  />
                </div>
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-2.5 py-1.5 bg-white border border-black/10 rounded-lg text-xs font-semibold"
                  >
              ...
              <div className="grid grid-cols-2 gap-3 pt-2 border-t border-black/5">
                <div>
                  <label className="font-bold text-gray-700 block mb-1">Rating Stars (0 - 5)</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="font-bold text-gray-700 block mb-1">Review Count</label>
                  <input
                    type="text"
          ...
          {schemaType === 'FAQPage' && (
            <div className="space-y-3 bg-gray-50/60 p-4 border border-black/5 rounded-xl text-xs">
              <div className="flex items-center justify-between mb-2">
                <span className="font-bold text-gray-700">FAQ Pairs ({faqItems.length})</span>
                <button
                  type="button"
              ...
              {faqItems.map((item, idx) => (
                <div key={idx} className="bg-white p-3 border border-black/10 rounded-lg space-y-2 relative group">
                  <input
                    type="text"
                  ...
                  <textarea
                    ...
                    className="w-full px-2 py-1 border border-black/10 rounded text-xs text-gray-600 resize-none"
                  />
                  <button
                    type="button"
                    onClick={() => setFaqItems(faqItems.filter((_, i) => i !== idx))}
                    className="text-red-500 hover:text-red-700 text-xs font-bold flex items-center gap-1 pt-1"
                  >
          ...
          {schemaType !== 'Product' && schemaType !== 'FAQPage' && (
            <div className="p-4 bg-gray-50 border border-black/5 rounded-xl text-xs text-gray-600 space-y-2">
        ...
        <div className="space-y-2 flex flex-col justify-between">
          <div className="flex items-center justify-between">
            ...
            <button
              type="button"
              onClick={handleCopy}
              className="px-3 py-1 bg-white border border-black/10 rounded-lg text-xs font-semibold text-gray-700 hover:bg-gray-50 flex items-center gap-1.5 shadow-2xs transition-colors cursor-pointer"
            >
